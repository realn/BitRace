...
CFile::CFile(std::string filename, char * flags) : fp(NULL) {
  Open(filename, flags);
}
...
bool CFile::Open(std::string filename, char * flags) {
...
unsigned int CFile::Write(void * Data, unsigned int Size, unsigned int Count) {
  return (unsigned int)fwrite(Data, Size, Count, fp) * Size;
}

unsigned int CFile::Read(void * Data, unsigned int Size, unsigned int Count) {
...
bool CFile::FileExists(std::string filename) {
  if (filename.empty())
    return false;
...
std::string CFile::ReadTextFile(std::string filename) {
  CFile fp;
